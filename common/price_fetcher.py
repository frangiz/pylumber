...
class PriceFetcher:
    ...
    def get_woody_price(self, url: str) -> float:
        ...
        id = id = soup.find("div", class_="inner-price base-unit").parent.get(
            "data-partnersku"
        )
    ...
    def get_byggmax_price(self, url: str) -> float:
        ...
        for script_tag in soup.find_all("script", type="application/ld+json"):
            ...
            if type(tag_data) != list:
                continue
            product_types = list(
                filter(lambda item: item.get("@type", None) == "Product", tag_data)
            )

            def fix_url(the_url):
                quoted_chars = {"å": quote("å"), "ä": quote("ä"), "ö": quote("ö")}
                for k, v in quoted_chars.items():
                    the_url = the_url.replace(k, v)
                return the_url

            # We have one product on the page, so return the price.
            if len(product_types) == 1 and fix_url(
                product_types[0]["offers"].get("url", None)
            ).startswith(url):
    ...
    def get_bauhaus_price(self, url: str) -> float:
        ...
        return float(
            base_tag.find(
                ...
                and tag.attrs.get("itemprop", "") == "price"
            ).attrs.get("content")
        )
